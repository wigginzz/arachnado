...
export default class ImageLayout extends React.Component {
    ...
    getItemStyle(item) {
        ...
        const normalizedHeight = (columnWidth / 200) * 200;
        this.columnHeights[shortestColumnIndex] += normalizedHeight + gutter + 5;
        return {
            left: `${left}px`,
            top: `${top}px`,
            position: `absolute`,
     ...
     */
	 //<span style={{display:'block',whiteSpace:'pre-wrap'}}>{item.title}</span>	
    renderItem(item, index) {
        return (
			
			<div style={this.getItemStyle(item)}>
